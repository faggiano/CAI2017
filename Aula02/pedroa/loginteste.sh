...
menu(){
	while : ; do
	LOGIN=$(dialog --stdout --title "Login"	--inputbox "Digite o seu usuário" 0 0)
	PASSW=$(dialog --stdout --title "Password" --passwordbox "Digite a sua senha" 0 0)
	[[ $LOGIN == "" ]] && [[ $PASSW == "" ]] && incorreto && menu ;
  [[ $LOGIN == $LOGIN ]] && [[ $PASSW == "" ]] && incorreto && menu ;
  [[ $LOGIN == "" ]] && [[ $PASSW == $PASSW ]] && incorreto && menu ;
		echo
